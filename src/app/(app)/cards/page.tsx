...
import { ChevronLeft, Plus, CreditCard, Edit3, Eye, EyeOff } from 'lucide-react'
...
export default function CardsListPage() {
  ...
  const renderCardLogo = (cardFlag: string) => {
    const IconComp = getDynamicIcon('credit-card')
          ...
          <div className="px-4 space-y-3">
            {cards.length === 0 ? (
              ...
              cards.map((card, index) => (
                ...
                >
                  <div className="p-4 cursor-pointer" onClick={() => router.push(`/cards/${card.id}`)}>
                    <div className="flex items-center gap-3 mb-3">
                      <div className="w-14 h-10 rounded-lg flex items-center justify-center text-white font-bold text-sm shadow-sm relative overflow-hidden"
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-[14px] font-bold text-gray-800 dark:text-gray-200 truncate">{card.name}</p>
                        <p className="text-[11px] text-gray-500 dark:text-gray-400">{card.flag || 'Cartão'} {card.last_four ? `•••• ${card.last_four}` : ''}</p>
                      </div>
                      <button onClick={(e) => { e.stopPropagation(); router.push(`/cards/${card.id}/edit`) }}
                    ...
                    <div className="mb-2">
                      ...
                      <div className="w-full bg-gray-100 dark:bg-slate-700 rounded-full h-2 overflow-hidden">
                        <div className={`h-full rounded-full transition-all duration-500 ${getProgressColor(card.percentUsed)}`}
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase">Fatura atual</p>
                      ...
                      <div className="text-right">
                        <p className="text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase">Vencimento</p>
