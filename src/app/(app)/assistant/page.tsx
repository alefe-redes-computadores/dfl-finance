...
function AssistantContent() {
  ...
  // 🔥 NOVO: Estados para a Chave da API
    ...
    <div ref={containerRef} className="max-w-md mx-auto min-h-screen bg-[#f8f9fa] dark:bg-slate-900 pb-28 font-sans px-4 pt-6 transition-colors duration-300">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <button
          ...
          className={`p-2 -mr-2 transition-colors ${showSettings ? 'text-teal-600' : 'text-gray-400 hover:text-teal-600'}`}
        >
          <Settings size={22} />
        </button>
      </div>
      ...
      {/* 🔥 NOVO: Modal/Painel de Configuração da Chave */}
      {showSettings && (
        <div className="bg-white dark:bg-slate-800 rounded-2xl p-5 shadow-sm border border-gray-100 dark:border-slate-700 mb-6 animate-in fade-in slide-in-from-top-4 duration-300">
          <label className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase mb-2 block">Chave da API Gemini</label>
          <input
            value={apiKey}
            onChange={e => setApiKey(e.target.value)}
            type="password"
            placeholder="Cole sua chave aqui"
            className="w-full bg-gray-50 dark:bg-slate-700 p-3.5 rounded-xl outline-none text-sm mb-3 text-gray-800 dark:text-gray-200 border border-gray-100 dark:border-slate-600 focus:border-teal-500 transition-colors"
          />
          <button
          ...
          {settingsMessage.text && (
            <div className={`mt-3 flex items-center gap-2 text-sm ${settingsMessage.type === 'error' ? 'text-red-500' : 'text-emerald-500'}`}>
          ...
          <p className="text-[11px] text-gray-500 dark:text-gray-400 mt-3 text-center">
            Obtenha sua chave em: <a href="https://aistudio.google.com/app/apikeys" target="_blank" rel="noreferrer" className="text-teal-600 dark:text-teal-400 hover:underline">aistudio.google.com</a>
          </p>
        </div>
      )}
