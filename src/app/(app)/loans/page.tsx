    ...
    <div className="flex flex-col h-[100dvh] bg-slate-50 dark:bg-slate-950">
      ...
      <div className="sticky top-0 z-30 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 shadow-sm px-4 pb-3">
        <div className="flex items-center justify-between pt-4 mb-3">
          <div><h1 className="text-xl font-black text-slate-800 dark:text-slate-100">Empréstimos</h1><p className="text-xs font-semibold text-slate-500 dark:text-slate-400">{appMode === "personal_only" ? "Pessoais" : "Empresariais e Pessoais"}</p></div>
          <div className="flex items-center gap-2">
            ...
            <button onClick={() => router.push("/loans/new")} className="p-2 rounded-xl bg-teal-500 hover:bg-teal-600 text-white shadow-md shadow-teal-500/20 transition-all active:scale-95"><Plus size={18} /></button>
          </div>
        </div>
        {showSearch && (
          <div className="relative mb-2"><Search size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" /><input type="text" placeholder="Buscar empréstimo..." value={search} onChange={(e) => setSearch(e.target.value)} className="w-full pl-9 pr-4 py-2.5 rounded-xl bg-slate-100 dark:bg-slate-800 text-sm font-semibold outline-none focus:ring-2 focus:ring-teal-500/50 placeholder:text-slate-400" /></div>
        )}
        <div className="flex gap-2 overflow-x-auto scrollbar-hide">
          ...
          <button onClick={() => setSortBy("due_date")} className={`px-3 py-1.5 rounded-full text-xs font-bold whitespace-nowrap transition-colors ${sortBy === "due_date" ? "bg-teal-500 text-white" : "bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400"}`}>Vencimento</button>
        </div>
      </div>
      ...
      <div ref={scrollRef} onTouchStart={handleTouchStart} onTouchMove={handleTouchMove} className="flex-1 overflow-y-auto px-4 pt-3 pb-24">
        {loading ? <Skeleton count={4} /> : sortedLoans.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20"><HandCoins size={48} className="text-slate-300 dark:text-slate-700 mb-3" /><p className="text-slate-500 dark:text-slate-400 font-semibold">Nenhum empréstimo</p></div>
        ) : (
                  ...
                  <button onClick={() => router.push(`/loans/${loan.id}`)} className="w-full p-4 text-left hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                    <div className="flex items-start justify-between">
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2">{getStatusBadge(loan.status)}<span className="text-xs text-slate-500 dark:text-slate-400">{formatDate(loan.date)}</span></div>
                        {loan.direction && <span className={`inline-block mt-1 text-xs font-semibold px-2 py-0.5 rounded-full ${loan.direction === "lent" ? "bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-300" : "bg-orange-100 dark:bg-orange-900/30 text-orange-700 dark:text-orange-300"}`}>{loan.direction === "lent" ? "Emprestei" : "Peguei"}</span>}
                      </div>
                      <div className="text-right flex-shrink-0">
                        <p className="font-black text-lg text-slate-800 dark:text-slate-200">{formatCurrency(loan.amount || 0)}</p>
                        {loan.status === "active" && <p className="text-xs text-slate-500 dark:text-slate-400">Resta: {formatCurrency(Math.max(0, remaining))}</p>}
                      </div>
                    </div>
                    {loan.lender && <p className="text-xs text-slate-500 dark:text-slate-400 mt-2">{loan.direction === "lent" ? "Devedor" : "Credor"}: {loan.lender}</p>}
                    {payments.length > 0 && (
                      <div className="mt-3 pt-3 border-t border-slate-100 dark:border-slate-800">
                        <div className="flex items-center justify-between mb-2"><span className="text-xs font-bold text-slate-600 dark:text-slate-400">Pagamentos ({payments.length})</span><span className="text-xs font-bold text-teal-600 dark:text-teal-400">{formatCurrency(totalPaid)}</span></div>
                        <div className="space-y-1.5 max-h-28 overflow-y-auto">
                          {payments.slice(0, isExpanded ? undefined : 2).map((p: Payment) => (
                            <div key={p.id} className="flex items-center justify-between text-xs bg-slate-50 dark:bg-slate-800 rounded-lg px-2 py-1"><span className="text-slate-600 dark:text-slate-400">{formatDate(p.date)}</span><span className="font-bold text-teal-600 dark:text-teal-400">{formatCurrency(p.amount)}</span></div>
                          ))}
                        </div>
                        {payments.length > 2 && <button onClick={(e) => { e.stopPropagation(); setExpandedId(isExpanded ? null : loan.id) }} className="w-full text-center text-xs text-teal-500 hover:text-teal-600 font-semibold mt-1 py-1">{isExpanded ? "Ver menos" : `Ver mais ${payments.length - 2} pagamentos`}<ChevronDown size={12} className={`inline ml-1 transition-transform ${isExpanded ? "rotate-180" : ""}`} /></button>}
                      </div>
                    )}
                  </button>
