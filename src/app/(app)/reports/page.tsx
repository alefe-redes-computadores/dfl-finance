...
// 🔥 NOVO: Importando os serviços Client-Side
...
function ExportModal({ isOpen, onClose, onExport, isExporting }: { isOpen: boolean; onClose: () => void; onExport: (format: 'pdf' | 'csv') => void, isExporting: boolean }) {
    ...
    <div className="fixed inset-0 z-[200] flex items-end justify-center bg-black/50" onClick={onClose}>
      <div className="bg-white dark:bg-slate-800 w-full max-w-lg rounded-t-3xl p-6 animate-in slide-in-from-bottom-10 duration-300" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex gap-3">
          {/* O PDF no momento não está no escopo do novo exportService, então mantemos a visualização, mas a ação principal é CSV */}
          <button
          ...
          <button
            ...
            disabled={isExporting}
            className="flex-1 bg-gray-200 dark:bg-slate-700 text-gray-700 dark:text-gray-300 py-4 rounded-xl font-bold text-sm hover:bg-gray-300 dark:hover:bg-slate-600 transition-colors flex items-center justify-center gap-2 disabled:opacity-50"
          >
...
export default function ReportsPage() {
  ...
  // Controle de estado para a exportação
  ...
  // 🔥 A MÁGICA DA EXPORTAÇÃO ACONTECE AQUI
  const handleExport = async (format: 'pdf' | 'csv') => {
    if (!user?.id) return
    
    if (format === 'pdf') {
      showToast('Funcionalidade de PDF será disponibilizada em breve!', 'info')
    ...
    setIsExporting(true)
    try {
      // Usa o número de meses selecionado nos botões (1, 3, 6, 12) em dias aproximados
