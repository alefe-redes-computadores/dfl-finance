...
import {
  ...
  Wallet, Layers3, BadgePercent, Landmark
} from "lucide-react"
...
function FinancingDetailContent() {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case "active":
        return <span className="flex items-center gap-1.5 text-[12px] font-bold text-blue-700 dark:text-blue-300 bg-blue-50 dark:bg-blue-900/30 px-3 py-1.5 rounded-full"><Clock size={12} /> Ativo</span>
      case "paid":
        return <span className="flex items-center gap-1.5 text-[12px] font-bold text-teal-700 dark:text-teal-300 bg-teal-50 dark:bg-teal-900/30 px-3 py-1.5 rounded-full"><CheckCircle2 size={12} /> Quitado</span>
      case "overdue":
        return <span className="flex items-center gap-1.5 text-[12px] font-bold text-red-700 dark:text-red-300 bg-red-50 dark:bg-red-900/30 px-3 py-1.5 rounded-full"><AlertTriangle size={12} /> Atrasado</span>
      default:
        return <span className="text-[12px] font-bold text-gray-500 bg-gray-100 dark:bg-slate-800 px-3 py-1.5 rounded-full">{status}</span>
    }
  }
  ...
  if (notFound) {
    return (
      <div className="flex flex-col h-[100dvh] bg-gray-50 dark:bg-slate-900">
        <div className="sticky top-0 z-30 bg-white/90 dark:bg-slate-900/90 backdrop-blur-xl border-b border-gray-100 dark:border-slate-800 px-4 pt-6 pb-4">
          <button onClick={() => router.back()} className="p-2 rounded-full bg-gray-100 dark:bg-slate-800">
            <ArrowLeft size={20} />
          </button>
      ...
      <div className="sticky top-0 z-30 bg-white/90 dark:bg-slate-900/90 backdrop-blur-xl border-b border-gray-100 dark:border-slate-800 shadow-sm px-4 pt-6 pb-4">
        <div className="flex items-center justify-between gap-3">
          ...
          <div className="flex gap-1 shrink-0">
            <button
              onClick={() => { vibrate([5]); router.push(`/financings/new?edit=${financingId}`) }}
      ...
      >
        <section className="rounded-[30px] bg-gradient-to-br from-teal-600 to-teal-700 text-white p-6 shadow-[0_16px_40px_rgba(13,148,136,0.28)]">
          <div className="flex items-start justify-between gap-3">
            <div>
              <p className="text-[12px] font-semibold text-teal-50/90">Valor restante</p>
        ...
        <section className="bg-white/92 dark:bg-slate-800/92 border border-gray-100/80 dark:border-slate-700/70 rounded-[28px] p-4">
          <div className="flex items-center gap-3 mb-4">
            <div className="w-12 h-12 rounded-[18px] bg-teal-50 dark:bg-teal-900/20 flex items-center justify-center">
          ...
          <div className="grid grid-cols-1 gap-3">
            <div className="grid grid-cols-3 gap-3">
              <div className="rounded-[20px] bg-gray-50 dark:bg-slate-700/35 border border-gray-100 dark:border-slate-700/60 p-3.5">
                <div className="flex items-center gap-2 text-gray-500 dark:text-gray-400 mb-2">
                  <Layers3 size={14} />
                  <span className="text-[11px] font-semibold">Parcelas</span>
                </div>
                <p className="text-[17px] font-black text-gray-800 dark:text-gray-100">
                  {financingData.installments_count}
                </p>
              </div>
              ...
              <div className="rounded-[20px] bg-gray-50 dark:bg-slate-700/35 border border-gray-100 dark:border-slate-700/60 p-3.5">
                <div className="flex items-center gap-2 text-gray-500 dark:text-gray-400 mb-2">
                  <Wallet size={14} />
                  <span className="text-[11px] font-semibold">Parcela</span>
                </div>
                <p className="text-[14px] font-black text-gray-800 dark:text-gray-100 leading-tight">
              ...
              <div className="rounded-[20px] bg-gray-50 dark:bg-slate-700/35 border border-gray-100 dark:border-slate-700/60 p-3.5">
                <div className="flex items-center gap-2 text-gray-500 dark:text-gray-400 mb-2">
                  ...
                  <span className="text-[11px] font-semibold">Juros</span>
                </div>
                <p className="text-[17px] font-black text-gray-800 dark:text-gray-100">
            ...
            {financingData.first_due_date && (
              <div className="rounded-[20px] bg-gray-50 dark:bg-slate-700/35 border border-gray-100 dark:border-slate-700/60 px-4 py-3.5 flex items-center justify-between gap-3">
                <div className="flex items-center gap-2 text-gray-500 dark:text-gray-400">
                  <Calendar size={14} />
                  <span className="text-[12px] font-semibold">Primeiro vencimento</span>
                </div>
                <span className="text-[13px] font-bold text-gray-800 dark:text-gray-100">
            ...
            {financingData.notes && (
              <div className="rounded-[20px] bg-gray-50 dark:bg-slate-700/35 border border-gray-100 dark:border-slate-700/60 px-4 py-3.5">
                <p className="text-[11px] font-semibold text-gray-500 dark:text-gray-400 mb-1.5">Observações</p>
        ...
        <section className="bg-white/92 dark:bg-slate-800/92 border border-gray-100/80 dark:border-slate-700/70 rounded-[28px] p-4">
          <div className="flex items-center justify-between mb-4 gap-3">
            <div>
              <h3 className="font-bold text-[16px] text-gray-800 dark:text-gray-100">Parcelas</h3>
          ...
          {installmentsLoading ? (
            ...
            <div className="space-y-2.5">
                    ...
                    <div className="flex items-center justify-between gap-3">
                      <div className="flex items-center gap-3 min-w-0 flex-1">
                        <button
                          onClick={() => {
                            vibrate([10])
                            inst.paid ? handleUndoPayment(inst) : handlePayInstallment(inst)
                          }}
                        ...
                        <div className="min-w-0">
                          <p className={`text-[14px] font-bold truncate ${
                            inst.paid ? "text-teal-700 dark:text-teal-300" : "text-gray-800 dark:text-gray-100"
                          }`}>
                            Parcela #{inst.number}
                          </p>
                          <p className={`text-[12px] mt-0.5 ${
                            inst.paid ? "text-teal-700/80 dark:text-teal-300/80" : "text-gray-500 dark:text-gray-400"
                          }`}>
                            Vence {formatDate(inst.due_date)}
                            {inst.paid && inst.paid_date ? ` • Pago ${formatDate(inst.paid_date)}` : ""}
                          </p>
                        </div>
                      </div>
                      ...
                      <div className="flex flex-col items-end gap-1.5 shrink-0">
                        <span className={`font-black text-[14px] ${
                          inst.paid ? "text-teal-700 dark:text-teal-300" : "text-gray-700 dark:text-gray-200"
                        }`}>
                          {formatCurrency(inst.amount)}
                        </span>
                        <button
                          onClick={() => { vibrate([5]); setDeleteModal(inst.id) }}
              ...
              {installments.length > 5 && (
                <button
                  onClick={() => { vibrate([5]); setExpandedInstallments(!expandedInstallments) }}
          ...
          >
            ...
            <div className="flex items-center justify-between mb-2">
              ...
              <button
                ...
                className="text-gray-400 bg-gray-100 dark:bg-slate-700 p-2 rounded-full active:scale-95"
              >
                <X size={20} />
              </button>
            </div>
            ...
            <div className="flex gap-3">
              ...
              <button
                onClick={() => { vibrate([10, 50]); handleDeleteInstallment(deleteModal) }}
