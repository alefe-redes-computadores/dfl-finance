...
export default function FinancingsPage() {
  ...
  const { data: financings, loading, reload } = useLocalData({ table: 'financings' as any, filters: { context: effectiveContext } })
  const { data: allInstallments } = useLocalData({ table: 'transactions' as any, filters: { context: effectiveContext, type: 'financing_installment' } })
  ...
  const handleDelete = async () => {
    ...
    try {
      await db.transaction('rw', 'financings', 'transactions', 'syncQueue', async () => {
        ...
        await db.table('syncQueue').add({ table: 'financings', operation: 'delete', record_id: deleteModal, user_id: user.id, created_at: new Date().toISOString() })
      })
  ...
  const getAssetIcon = (type: string) => {
    switch (type) {
      case "vehicle": return <Car size={16} className="text-teal-500 flex-shrink-0" />
      case "property": return <Home size={16} className="text-teal-500 flex-shrink-0" />
      default: return <Percent size={16} className="text-teal-500 flex-shrink-0" />
    }
  }
    ...
    <div className="flex flex-col h-[100dvh] bg-slate-50 dark:bg-slate-950">
      ...
      <div className="sticky top-0 z-30 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 shadow-sm px-4 pb-3">
        <div className="flex items-center justify-between pt-4 mb-3">
          <div><h1 className="text-xl font-black text-slate-800 dark:text-slate-100">Financiamentos</h1><p className="text-xs font-semibold text-slate-500 dark:text-slate-400">{appMode === "personal_only" ? "Pessoais" : "Empresariais e Pessoais"}</p></div>
          <div className="flex items-center gap-2">
            ...
            <button onClick={() => router.push("/financings/new")} className="p-2 rounded-xl bg-teal-500 hover:bg-teal-600 text-white shadow-md shadow-teal-500/20 transition-all active:scale-95"><Plus size={18} /></button>
          </div>
        </div>
        {showSearch && (
          <div className="relative mb-2"><Search size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" /><input type="text" placeholder="Buscar financiamento..." value={search} onChange={(e) => setSearch(e.target.value)} className="w-full pl-9 pr-4 py-2.5 rounded-xl bg-slate-100 dark:bg-slate-800 text-sm font-semibold outline-none focus:ring-2 focus:ring-teal-500/50 placeholder:text-slate-400" /></div>
        )}
      ...
      <div ref={scrollRef} onTouchStart={handleTouchStart} onTouchMove={handleTouchMove} className="flex-1 overflow-y-auto px-4 pt-3 pb-24">
        {loading ? <Skeleton count={4} /> : sortedFinancings.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20"><Percent size={48} className="text-slate-300 dark:text-slate-700 mb-3" /><p className="text-slate-500 dark:text-slate-400 font-semibold">Nenhum financiamento</p></div>
        ) : (
                  ...
                  <button onClick={() => router.push(`/financings/${fin.id}`)} className="w-full p-4 text-left hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                    <div className="flex items-start justify-between">
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2">{getStatusBadge(fin.status)}<span className="text-xs text-slate-500 dark:text-slate-400">{formatDate(fin.start_date)}</span></div>
                    ...
                    {installments.length > 0 && (
                      <div className="mt-3 pt-3 border-t border-slate-100 dark:border-slate-800">
                        <div className="flex items-center justify-between mb-2"><span className="text-xs font-bold text-slate-600 dark:text-slate-400">Parcelas ({paidInstallments.length}/{installments.length})</span><span className="text-xs font-bold text-teal-600 dark:text-teal-400">{formatCurrency(totalPaid)}</span></div>
                        <div className="space-y-1.5 max-h-28 overflow-y-auto">
                          {installments.slice(0, isExpanded ? undefined : 3).map((inst: Installment) => (
                            <div key={inst.id} className="flex items-center justify-between text-xs bg-slate-50 dark:bg-slate-800 rounded-lg px-2 py-1">
                              <div className="flex items-center gap-2"><span className={`w-2 h-2 rounded-full ${inst.paid ? "bg-teal-500" : "bg-slate-300 dark:bg-slate-600"}`} /><span className="text-slate-600 dark:text-slate-400">#{inst.number} — {formatDate(inst.due_date)}</span></div>
                        ...
                        {installments.length > 3 && (
                          <button onClick={(e) => { e.stopPropagation(); setExpandedId(isExpanded ? null : fin.id) }} className="w-full text-center text-xs text-teal-500 hover:text-teal-600 font-semibold mt-1 py-1">
                            {isExpanded ? "Ver menos" : `Ver todas (${installments.length})`}<ChevronDown size={12} className={`inline ml-1 transition-transform ${isExpanded ? "rotate-180" : ""}`} />
                          </button>
                        )}
                      </div>
                    )}
      ...
      {deleteModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={() => setDeleteModal(null)}>
          <div className="bg-white dark:bg-slate-900 rounded-2xl p-6 max-w-sm w-full shadow-xl" onClick={(e) => e.stopPropagation()}>
            ...
            <p className="text-sm text-slate-600 dark:text-slate-400 mb-6">Tem certeza que deseja excluir este financiamento e todas as suas parcelas?</p>
