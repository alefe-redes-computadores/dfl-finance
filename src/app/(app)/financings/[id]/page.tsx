...
export default function FinancingDetailPage() {
  ...
  // 🔥 CORRIGIDO: Pagar parcela com db + sync
  const handlePayInstallment = async (installment: Installment) => {
    ...
    try {
      ...
      await addToSyncQueue(user.id, 'transactions', 'update', installment.id, updateData)
      ...
      if (allPaid && financingData?.status !== "paid") {
        ...
        await addToSyncQueue(user.id, 'financings', 'update', financingId, statusUpdate)
      }
  ...
  // 🔥 CORRIGIDO: Desfazer pagamento com db + sync
  ...
  // 🔥 CORRIGIDO: Excluir parcela com db + sync
  const handleDeleteInstallment = async (installmentId: string) => {
    if (!user) return
    try {
      await db.table('transactions').delete(installmentId)
      await addToSyncQueue(user.id, 'transactions', 'delete', installmentId, { id: installmentId })
  ...
  // 🔥 CORRIGIDO: Excluir financiamento com db + sync
  const handleDeleteFinancing = async () => {
    ...
    try {
      ...
      await addToSyncQueue(user.id, 'financings', 'delete', financingId, { id: financingId })
