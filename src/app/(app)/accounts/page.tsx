    ...
    <div className="max-w-md mx-auto min-h-screen bg-[#f8f9fa] pb-24 font-sans relative">
      ...
      <div className="bg-[#f8f9fa] px-4 pt-6 pb-2 sticky top-0 z-10">
        <div className="flex items-center justify-between mb-6">
          ...
          <h1 className="text-[17px] font-bold text-gray-800">Contas</h1>
          <button onClick={() => setShowForm(true)} className="p-2 -mr-2 text-teal-700 hover:text-teal-800 transition-colors">
            <Plus size={24} />
          </button>
        </div>
      ...
      <div className="px-4 mt-6">
        ...
        <div className="bg-white rounded-[24px] p-6 shadow-[0_2px_10px_rgba(0,0,0,0.02)] border border-gray-50 mb-6 text-center flex flex-col items-center">
          <div className="flex items-center gap-2 mb-2">
            <span className="text-[11px] text-gray-400 font-bold uppercase tracking-widest">Saldo total</span>
            <button onClick={() => setHideBalance(!hideBalance)} className="text-gray-400 hover:text-gray-600 transition-colors">
              {hideBalance ? <EyeOff size={14} /> : <Eye size={14} />}
            </button>
          </div>
          <p className="text-[32px] font-light text-gray-800">
        ...
        {loading ? (
          ...
          <div className="text-center py-20 text-gray-400 text-[14px]">Nenhuma conta encontrada.</div>
        ) : (
            ...
            {accounts.map((acc) => {
              ...
              return (
                <div 
                  ...
                  className="bg-white p-4 rounded-[20px] shadow-[0_2px_10px_rgba(0,0,0,0.02)] border border-gray-50 flex items-center justify-between cursor-pointer hover:bg-gray-50 transition-colors"
                >
                  <div className="flex items-center gap-4">
                    <GripVertical className="text-gray-300 cursor-grab hover:text-gray-500 transition-colors" size={18} />
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-gray-400 font-bold uppercase tracking-wider mt-0.5">Conta Corrente</p>
                    </div>
                  </div>
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm flex items-end sm:items-center justify-center p-0 sm:p-4" onClick={() => setShowForm(false)}>
          <div className="bg-white rounded-t-[32px] sm:rounded-[24px] w-full max-w-sm p-6 shadow-2xl animate-in slide-in-from-bottom-10" onClick={e => e.stopPropagation()}>
            
            <div className="flex justify-between items-center mb-8">
              <h2 className="font-bold text-[18px] text-gray-800">Nova Conta</h2>
              <button onClick={() => setShowForm(false)} className="p-2 -mr-2 text-gray-400 hover:text-gray-600 transition-colors"><X size={20}/></button>
            </div>

            <label className="block text-[11px] font-bold text-gray-400 uppercase tracking-wider mb-2">Nome da Instituição</label>
            <input value={name} onChange={e => setName(e.target.value)} placeholder="Ex: Nubank, Inter" className="w-full bg-transparent border-b-2 border-gray-100 py-3 mb-8 text-[16px] outline-none focus:border-teal-600 font-bold text-gray-800 transition-colors" />
            
            <label className="block text-[11px] font-bold text-gray-400 uppercase tracking-wider mb-3 block">Cor da Conta</label>
            ...
            <div className="bg-gray-50 rounded-2xl p-4 flex items-center gap-2 mb-8 border border-gray-100">
               <span className="text-gray-400 font-bold text-lg">R$</span>
               <input type="text" inputMode="numeric" value={displayBalance} onChange={handleBalanceChange} placeholder="0,00" className="w-full bg-transparent text-2xl font-light text-gray-800 outline-none" />
            </div>
            
            {/* Toggle de Permitir Saldo Negativo (Cheque Especial) */}
            <label className="flex items-center justify-between mb-8 cursor-pointer bg-gray-50 p-4 rounded-[20px] border border-gray-100">
              <div>
                <p className="text-[13px] font-bold text-gray-800">Permitir Saldo Negativo</p>
                <p className="text-[10px] text-gray-400 font-medium">Conta possui limite / cheque especial</p>
              </div>
              <div className="relative">
                <input type="checkbox" className="sr-only" checked={allowNegative} onChange={(e) => setAllowNegative(e.target.checked)} />
                <div className={`block w-11 h-6 rounded-full transition-colors ${allowNegative ? 'bg-teal-700' : 'bg-gray-200'}`}></div>
