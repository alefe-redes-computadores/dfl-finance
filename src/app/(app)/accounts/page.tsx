...
export default function AccountsPage() {
  ...
  async function handleDelete(id: string) {
    if (!confirm('Tem certeza? Isso apagará a conta e as transações vinculadas.')) return
      ...
      <div className="max-w-md mx-auto min-h-screen bg-slate-50 pt-6 pb-24 font-sans animate-in slide-in-from-right-4 duration-200">
        <div className="flex items-center justify-between px-4 mb-6">
          ...
          <div className="flex gap-4">
            <button onClick={() => { setEditingId(selectedAccount.id); setName(selectedAccount.name); setColor(selectedAccount.color); setShowForm(true); }}><Edit2 size={22} /></button>
          </div>
        </div>
        <div className="flex flex-col items-center px-4 mb-8">
          ...
          <p className="text-gray-500 mb-2">Saldo: <span className={getBalanceStyle(Number(selectedAccount.balance))}>{formatCurrency(Number(selectedAccount.balance))}</span></p>
        ...
        <div className="flex px-4 mb-8 border-b border-gray-200 pb-4">
            <div className="flex-1 text-center"><p className="text-[10px] text-gray-500 uppercase">Entradas</p><p className="font-bold text-emerald-600">{formatCurrency(accSummary.income)}</p></div>
            <div className="flex-1 text-center"><p className="text-[10px] text-gray-500 uppercase">Saídas</p><p className="font-bold text-red-500">{formatCurrency(accSummary.expense)}</p></div>
        </div>
        <div className="px-4 space-y-3">
          {accTransactions.map(t => (
            <div key={t.id} className="bg-white rounded-2xl p-4 flex justify-between shadow-sm border border-gray-100">
              <div><p className="font-bold text-sm">{t.description || t.categories?.name}</p><p className="text-[10px] text-gray-400">{format(new Date(t.date + 'T12:00:00'), "d 'de' MMM", { locale: ptBR })}</p></div>
  ...
  // TELA PRINCIPAL (LISTA + FORM + CONTEXTO)
  return (
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl w-full max-w-sm p-6 shadow-2xl">
            <h2 className="font-bold mb-4">{editingId ? 'Editar Conta' : 'Nova Conta'}</h2>
            ...
            <input value={displayBalance} onChange={handleBalanceChange} placeholder="Saldo" className="w-full bg-gray-100 p-3 rounded-xl mb-4 font-bold" />
            <div className="flex gap-2">
                <button onClick={handleSave} className="flex-1 bg-emerald-900 text-white py-3 rounded-xl font-bold">Salvar</button>
                {editingId && <button onClick={() => handleDelete(editingId)} className="w-12 bg-red-100 text-red-600 rounded-xl flex items-center justify-center"><Trash2 size={20}/></button>}
            </div>
          </div>
        </div>
      )}
