...
function GoalDetailContent() {
  ...
  const handleContribAmountChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!digits) { setContribAmount(''); setContribAmountNum(0); return }
  ...
  return (
    <div ref={containerRef} className="max-w-md mx-auto min-h-screen bg-gray-50 dark:bg-slate-900 pb-24 font-sans transition-colors duration-300">
      ...
      <div className="px-4 pt-6 space-y-4 animate-in fade-in duration-300">
        <div className="flex justify-center mb-2">
          <span className={`text-[11px] font-bold px-3 py-1.5 rounded-full flex items-center gap-1.5 shadow-sm border ${isCompleted ? 'bg-emerald-50 text-emerald-600 border-emerald-200 dark:bg-emerald-900/30 dark:border-emerald-800' : isOverdue ? 'bg-red-50 text-red-600 border-red-200 dark:bg-red-900/30 dark:border-red-800' : 'bg-teal-50 text-teal-700 border-teal-200 dark:bg-teal-900/30 dark:border-teal-800'}`}>
            {isCompleted && <CheckCircle size={12} />}{isOverdue && <AlertTriangle size={12} />}{isCompleted ? 'Concluída' : isOverdue ? 'Atrasada' : `${daysLeft} dias restantes`}
          </span>
        </div>
        ...
        <div className="bg-white dark:bg-slate-800 rounded-[28px] p-6 shadow-sm border border-gray-50 dark:border-slate-700/50 mb-4">
          <div className="flex items-center gap-4 mb-6 mt-1">
            ...
            <div>
              <p className="font-black text-[18px] text-gray-800 dark:text-gray-100 leading-tight">{goal.name}</p>
              <p className="text-[11px] font-bold uppercase tracking-widest text-gray-400 dark:text-gray-500 mt-1">{goal.category ? `Meta para ${goal.category}` : 'Meta geral'}</p>
            </div>
          </div>
          ...
          <p className={`text-[12px] font-bold text-right ${isOverdue ? 'text-red-500' : 'text-gray-400 dark:text-gray-500'}`}>{percent.toFixed(1)}% concluído</p>
        </div>

        {!isCompleted && (
          <button onClick={() => { vibrate([5]); setShowContributionModal(true); }} className="w-full bg-teal-600 hover:bg-teal-700 text-white py-4 rounded-[24px] font-bold text-[15px] shadow-lg shadow-teal-600/30 active:scale-[0.98] transition-transform flex items-center justify-center gap-2 mb-4">
        ...
        <div className="bg-white dark:bg-slate-800 rounded-[28px] p-6 shadow-sm border border-gray-50 dark:border-slate-700/50">
          <h3 className="font-bold text-[16px] text-gray-800 dark:text-gray-100 mb-4">Histórico de Contribuições</h3>
          {transactions.length === 0 ? (
            ...
            <div className="space-y-2">
              {transactions.map((tx: any) => {
                ...
                return (
                  <div key={tx.id} className="flex items-center justify-between p-3.5 bg-gray-50 dark:bg-slate-700/40 rounded-[20px] transition-colors border border-gray-100 dark:border-slate-700/50">
                    <div className="flex items-center gap-4 flex-1 min-w-0">
                      <div className="w-10 h-10 rounded-[14px] flex items-center justify-center bg-emerald-100 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400"><IconTx size={18} /></div>
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-1.5"><p className="text-[14px] font-bold text-gray-800 dark:text-gray-200 truncate">{tx.description || 'Contribuição'}</p>{attachmentIcon && <span className="shrink-0">{attachmentIcon}</span>}</div>
                        <p className="text-[11px] font-medium text-gray-400 dark:text-gray-500 mt-0.5">{format(new Date(tx.date), "dd 'de' MMM yyyy", { locale: ptBR })}</p>
                      </div>
                    </div>
                    <p className={`text-[15px] font-black flex-shrink-0 ${isIncome ? 'text-emerald-600' : 'text-red-500'}`}>{isIncome ? '+' : '-'} {formatCurrency(Number(tx.amount) || 0)}</p>
                  </div>
                )
              })}
            </div>
      ...
      {/* Modal Registrar Contribuição - Bottom Sheet Premium */}
        ...
        <div className="fixed inset-0 z-[600] flex items-end justify-center" onClick={() => setShowContributionModal(false)}>
          ...
          <div className="relative w-full max-w-lg bg-white dark:bg-slate-800 rounded-t-[32px] p-6 shadow-[0_-8px_30px_rgba(0,0,0,0.12)] animate-in slide-in-from-bottom-8 duration-300" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex items-center justify-between mb-6">
              ...
              <button onClick={() => { vibrate([5]); setShowContributionModal(false); }} className="text-gray-400 bg-gray-100 dark:bg-slate-700 p-2 rounded-full active:scale-95"><X size={20} /></button>
            </div>
            ...
            <div className="space-y-4 mb-6">
              <div className="bg-gray-50 dark:bg-slate-700/40 border border-gray-100 dark:border-slate-700/50 rounded-[20px] p-4">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <input type="text" inputMode="numeric" value={contribAmount} onChange={handleContribAmountChange} className="w-full bg-transparent text-[24px] font-black text-gray-800 dark:text-gray-100 outline-none placeholder:text-gray-300 dark:placeholder:text-gray-600" placeholder="0,00" autoFocus />
                </div>
              </div>
              <div className="bg-gray-50 dark:bg-slate-700/40 border border-gray-100 dark:border-slate-700/50 rounded-[20px] p-4">
                <label className="text-[11px] font-bold text-gray-500 uppercase tracking-widest mb-2 block">Data</label>
                <input type="date" value={contribDate} onChange={e => setContribDate(e.target.value)} className="w-full bg-transparent text-[15px] font-bold text-gray-800 dark:text-gray-100 outline-none" />
              </div>
              <div className="bg-gray-50 dark:bg-slate-700/40 border border-gray-100 dark:border-slate-700/50 rounded-[20px] p-4">
                <label className="text-[11px] font-bold text-gray-500 uppercase tracking-widest mb-2 block">Observação (Opcional)</label>
                <input type="text" value={contribNote} onChange={e => setContribNote(e.target.value)} placeholder="Ex: Economia da semana" className="w-full bg-transparent text-[15px] font-bold text-gray-800 dark:text-gray-100 outline-none placeholder:text-gray-300 dark:placeholder:text-gray-600" />
              </div>
            </div>

            <button onClick={() => { vibrate([10, 50]); handleContribution(); }} disabled={contribAmountNum <= 0} className="w-full bg-teal-600 hover:bg-teal-700 text-white py-4 rounded-[24px] font-bold text-[16px] disabled:opacity-50 shadow-lg shadow-teal-600/30 active:scale-[0.98] transition-transform">
