...
import {
  ...
  ArrowLeftRight, TrendingUp as TrendingUpIcon
} from 'lucide-react'
...
export default function BudgetDetailPage() {
  ...
  return (
    <div className="max-w-md mx-auto min-h-screen bg-[#f8f9fa] dark:bg-slate-900 pb-20 font-sans px-4 pt-6 transition-colors duration-300">
      
      {/* Header */}
      <div className="flex items-center justify-between mb-6">
        <button onClick={() => router.push('/budgets')} className="p-2 -ml-2 text-gray-800 dark:text-gray-200">
          <ChevronLeft size={24} />
        </button>
        <h2 className="text-[18px] font-bold text-gray-800 dark:text-gray-100">{budget.name}</h2>
        <button onClick={() => router.push(`/budgets/new?edit=${budget.id}`)} className="p-2 -mr-2 text-teal-700 dark:text-teal-400">
          <Edit2 size={20} />
        </button>
      </div>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-[24px] p-5 shadow-sm border border-gray-50 dark:border-slate-700 mb-4">
        ...
        <p className="text-[11px] text-gray-400 dark:text-gray-500 font-medium text-right">{percent.toFixed(1)}% utilizado</p>
      </div>

      {/* Alerta de Projeção */}
      {projection && (
        <div className={`rounded-[20px] p-4 mb-4 shadow-sm border ${
          isOverBudget ? 'bg-red-50 dark:bg-red-900/20 border-red-200 dark:border-red-800 text-red-700 dark:text-red-400' :
          isWarning || (daysLeft !== null && daysLeft <= 7) ? 'bg-orange-50 dark:bg-orange-900/20 border-orange-200 dark:border-orange-800 text-orange-700 dark:text-orange-400' :
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-3 flex-1 min-w-0">
                    <div className="w-9 h-9 rounded-lg flex items-center justify-center" style={{ backgroundColor: `${tx.categories?.color || '#64748b'}20`, color: tx.categories?.color || '#64748b' }}>
                      <TxIconComp size={16} />
                    </div>
