    ...
    <div className="max-w-md mx-auto min-h-screen bg-[#f8f9fa] dark:bg-slate-900 pb-28 font-sans px-4 pt-6 transition-colors duration-300">
      ...
      {loading ? (
        <div className="space-y-4">
          ...
          <Skeleton variant="card" height="100px" count={2} />
        </div>
      ) : (
        <>
          ...
          <button onClick={() => setShowSimulator(!showSimulator)} className="w-full bg-teal-700 text-white py-3 rounded-xl font-bold mb-4 hover:bg-teal-800 transition-colors">
            Simular Cenário
          </button>
          ...
          {showSimulator && (
            <div className="bg-white dark:bg-slate-800 rounded-[24px] p-5 shadow-sm border border-gray-50 dark:border-slate-700 mb-6">
              ...
              <div className="flex gap-2 mb-4">
                <button onClick={() => setSimulatorType('expense')} className={`flex-1 py-2 rounded-full text-xs font-bold transition-colors ${simulatorType === 'expense' ? 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400' : 'bg-gray-100 dark:bg-slate-700 text-gray-500 dark:text-gray-400'}`}>Nova Despesa</button>
                <button onClick={() => setSimulatorType('income')} className={`flex-1 py-2 rounded-full text-xs font-bold transition-colors ${simulatorType === 'income' ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400' : 'bg-gray-100 dark:bg-slate-700 text-gray-500 dark:text-gray-400'}`}>Nova Receita</button>
              </div>
              <button onClick={handleSimulate} className="w-full bg-teal-700 text-white py-3 rounded-xl font-bold hover:bg-teal-800 transition-colors">Calcular Impacto</button>
            </div>
          )}
          ...
          {showSaveScenario && (
            <div className="bg-white dark:bg-slate-800 rounded-[24px] p-5 shadow-sm border border-gray-50 dark:border-slate-700 mb-6">
              ...
              <input type="text" value={saveScenarioName} onChange={(e) => setSaveScenarioName(e.target.value)} placeholder="Nome do cenário" className="w-full bg-gray-50 dark:bg-slate-700 rounded-xl p-3 outline-none text-sm text-gray-800 dark:text-gray-200 mb-4" />
              <button onClick={handleSaveScenario} disabled={savingScenario} className="w-full bg-teal-700 text-white py-3 rounded-xl font-bold hover:bg-teal-800 transition-colors disabled:opacity-50">{savingScenario ? 'Salvando...' : 'Salvar Cenário'}</button>
            </div>
          )}
          ...
          {scenarios.length > 0 && (
            <div className="bg-white dark:bg-slate-800 rounded-[24px] p-5 shadow-sm border border-gray-50 dark:border-slate-700">
              ...
              {scenarios.map((scenario: any) => (
                <div key={scenario.id} className={`p-3 rounded-xl mb-2 cursor-pointer ${selectedScenarioId === scenario.id ? 'bg-teal-50 dark:bg-teal-900/30' : 'hover:bg-gray-50 dark:hover:bg-slate-700'}`} onClick={() => setSelectedScenarioId(scenario.id === selectedScenarioId ? null : scenario.id)}>
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-[13px] font-bold text-gray-800 dark:text-gray-200">{formatCurrency(scenario.extra_expense > 0 ? scenario.extra_expense : scenario.extra_income)}</span>
                  </div>
                </div>
              ))}
            </div>
