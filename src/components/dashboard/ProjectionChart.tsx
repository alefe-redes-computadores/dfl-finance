  ...
  if (!data || data.length === 0) {
    return (
      <div className="bg-white dark:bg-slate-800 rounded-[28px] p-6 shadow-sm border border-gray-50 dark:border-slate-700/50 text-center py-10 transition-colors duration-300">
        ...
        <p className="text-gray-400 dark:text-gray-500 text-sm font-medium">Nenhum dado de projeção disponível</p>
      </div>
    )
  }
    ...
    if (active && payload && payload.length) {
      return (
        <div className="bg-white/95 dark:bg-slate-800/95 backdrop-blur-md p-4 rounded-[20px] shadow-[0_8px_30px_rgba(0,0,0,0.12)] border border-gray-100 dark:border-slate-700/50 z-50">
          <p className="font-bold text-gray-800 dark:text-gray-100 text-sm mb-3 border-b pb-2">{label}</p>
          <div className="space-y-2 text-[13px] font-medium">
            <p className="text-emerald-600 dark:text-emerald-400 flex justify-between gap-6">Otimista: <span className="font-bold">{formatCurrency(payload[0]?.value || 0)}</span></p>
            <p className="text-blue-600 dark:text-blue-400 flex justify-between gap-6">Realista: <span className="font-bold">{formatCurrency(payload[1]?.value || 0)}</span></p>
            <p className="text-red-500 dark:text-red-400 flex justify-between gap-6">Pessimista: <span className="font-bold">{formatCurrency(payload[2]?.value || 0)}</span></p>
          </div>
        </div>
      )
    }
      ...
      <div className="h-[240px]">
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={data} margin={{ top: 5, right: 0, left: -20, bottom: 0 }}>
            ...
            <XAxis dataKey="name" tick={{ fontSize: 11, fill: '#9ca3af' }} axisLine={false} tickLine={false} interval={2} />
            <YAxis tickFormatter={(v) => formatCurrency(v)} tick={{ fontSize: 11, fill: '#9ca3af' }} axisLine={false} tickLine={false} />
