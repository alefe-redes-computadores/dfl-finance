...
export default function NotificationCenter({ isOpen, onClose, notifications, onReadChange }: NotificationCenterProps) {
  ...
  // Lógica corrigida do markAsRead para atualizar banco local e SyncQueue
  ...
  const markAllAsRead = async () => {
    if (!user) return
    // Pega os IDs apenas das notificações não lidas
  ...
  // Filtra as notificações não lidas e agrupa apenas as ativas
  ...
  return (
    <>
      ...
      <div className="fixed inset-x-0 top-0 z-[110] mx-auto max-w-md pt-16 px-4">
        <div className="bg-white dark:bg-slate-800 rounded-3xl shadow-2xl overflow-hidden animate-in slide-in-from-top-2 duration-300 pointer-events-auto border border-gray-100 dark:border-slate-700">
            ...
            <div className="flex items-center gap-2">
              {unreadCount > 0 && (
                <button 
                  ...
                  disabled={processing}
                  className="flex items-center gap-1 px-3 py-1.5 bg-teal-50 dark:bg-teal-900/30 text-teal-700 dark:text-teal-400 rounded-full text-xs font-bold hover:bg-teal-100 dark:hover:bg-teal-900/50 transition-colors disabled:opacity-50"
                >
                  <Check size={14} />
                  Marcar todas
                </button>
              ...
              <div className="divide-y divide-gray-50 dark:divide-slate-700">
                {displayedGroups.map(group => (
                  <button
                    ...
                    disabled={processing}
                    className={`w-full flex items-center gap-3 px-5 py-4 text-left hover:bg-gray-50 dark:hover:bg-slate-700 transition-colors ${getBgColor(group.severity)}`}
                  >
                    <div className="relative w-9 h-9 rounded-full bg-white dark:bg-slate-800 flex items-center justify-center shadow-sm">
