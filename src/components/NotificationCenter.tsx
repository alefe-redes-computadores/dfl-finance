...
export default function NotificationCenter({ isOpen, onClose, notifications, onReadChange }: NotificationCenterProps) {
  ...
  return (
    <>
      <div className="fixed inset-0 z-[600] bg-black/40 backdrop-blur-sm transition-opacity" onClick={onClose} />
        ...
        <div className="bg-white/95 dark:bg-slate-800/95 backdrop-blur-xl rounded-[32px] shadow-[0_20px_60px_rgba(0,0,0,0.15)] overflow-hidden animate-in slide-in-from-top-6 duration-300 pointer-events-auto border border-white/20 dark:border-slate-700/50">
          ...
          <div className="flex items-center justify-between p-5 border-b border-gray-100 dark:border-slate-700/50">
            <div className="flex items-center gap-3">
              <div className="w-12 h-12 rounded-[18px] bg-teal-50 dark:bg-teal-900/30 flex items-center justify-center">
                <Bell size={22} className="text-teal-700 dark:text-teal-400" />
              </div>
              <div>
            ...
            <div className="flex items-center gap-2">
              {unreadCount > 0 && (
                <button 
                  ...
                  disabled={processing}
                  className="flex items-center gap-1.5 px-3 py-2 bg-teal-50 dark:bg-teal-900/30 text-teal-700 dark:text-teal-400 rounded-full text-[11px] font-bold hover:bg-teal-100 dark:hover:bg-teal-900/50 transition-colors disabled:opacity-50 active:scale-95"
                >
                  <Check size={14} />
                  Marcar todas
                </button>
          ...
          <div className="max-h-[55vh] overflow-y-auto">
            {grouped.length === 0 ? (
              ...
              <div className="divide-y divide-gray-50 dark:divide-slate-700/50 p-2 space-y-1">
                ...
                <div className="pt-2">
                  <button
                    ...
                    className="w-full flex items-center justify-center gap-2 p-4 text-teal-700 dark:text-teal-400 bg-teal-50/50 hover:bg-teal-50 dark:bg-teal-900/10 dark:hover:bg-teal-900/20 rounded-[24px] transition-colors font-bold text-[14px] active:scale-[0.98]"
                  >
                    <ExternalLink size={16} />
                    Ver Central Completa
                    {grouped.length > 5 && (
                      <span className="text-[11px] font-bold bg-teal-100 dark:bg-teal-900/40 px-2 py-0.5 rounded-full">
                        +{grouped.length - 5}
                      </span>
                    )}
                  </button>
